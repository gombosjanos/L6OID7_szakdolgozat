<template>
  <v-container fluid class="pa-4">
    ...
    <v-row>
      <v-col cols="12" md="5">
        ...
        <v-card class="mb-4">
          <v-card-title class="text-subtitle-1 font-weight-bold">Szerelõ adatai</v-card-title>
          <v-divider />
          <v-card-text>
      ...
      <v-col cols="12" md="7">
        <v-card>
          ...
          <v-card-text>
            ...
            <div v-if="offerRows.length===0" class="text-medium-emphasis">Még nincs elérhetõ árajánlat.</div>
            <template v-else>
  ...
  <button class="lb-nav lb-left" @click="prevImage" :disabled="images.length<=1" aria-label="Elõzõ">
    <svg viewBox="0 0 24 24" width="34" height="34" fill="currentColor" role="img" aria-hidden="true">
      <path d="M15.41 7.41 14 6l-6 6 6 6 1.41-1.41L10.83 12z"/>
    </svg>
  </button>
  <button class="lb-nav lb-right" @click="nextImage" :disabled="images.length<=1" aria-label="Következõ">
    <svg viewBox="0 0 24 24" width="34" height="34" fill="currentColor" role="img" aria-hidden="true">
...
async function openImage(img){
  ...
  try{
    ...
    if(!w){ snack('A kép megnyitását a böngészõ blokkolta.', 'warning') }
  }catch(e){
...
function displayStatus(s){
  const map={
    ...
    'javitas_kesz':'JavÃ­tás kész',
    'atadva_lezarva':'Átadva/Lezárva',
    'ajanlat_elfogadva':'Árajánlat elfogadva',
    'ajanlat_elutasitva':'Árajánlat elutasÃ­tva',
  }
  const k=(s||'').toLowerCase()
  return map[k] || s || '-'
}
...
async function oldOpenImage(img){
  ...
  try{
    ...
    if(!w){ snack('A kép megnyitása blokkolva lett a böngészõ által.', 'warning') }
  }catch(e){
    snack('Kép megnyitása nem sikerült.', 'error')
  }
}
...
function displayOfferStatus(s){
  const k=(s||'').toString().toLowerCase()
  const map={ elkuldve:'Elfogadásra vár', elfogadva:'Elfogadva', elutasitva:'ElutasÃ­tva' }
  return map[k] || '-'
}
